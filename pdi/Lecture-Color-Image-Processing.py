...
#%% Load a color image and visualize  in grayscale
img = cv2.imread(os.path.join(PATH_TO_IMAGES,'rgbcube_kBKG.png'))
plt.figure(figsize=(32,32))
plt.imshow((img))
plt.show()
...
#%% Load a color image and visualize each channel separately HSV
img = cv2.imread(os.path.join(PATH_TO_IMAGES,'rgbcube_kBKG.png'), cv2.IMREAD_GRAYSCALE)
# hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
h, s, v = cv2.split(hsv)
plt.figure(figsize=(16, 8))

plt.subplot(221); plt.title('Hue'); plt.imshow(h, cmap='hsv')
plt.subplot(222); plt.title('Saturation'); plt.imshow(s, cmap='hsv') 
plt.subplot(223); plt.title('Value'); plt.imshow(v, cmap='hsv')
plt.subplot(224); plt.title('Original'); plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
